    ...
    <div className="p-8 max-w-[1600px] mx-auto pb-24 h-full relative">
      ...
      {step === 6 && selectedDef && (
        <div className="animate-in fade-in slide-in-from-bottom-4 space-y-10 max-w-5xl mx-auto">
          <div className="text-center mb-8"><h2 className="text-4xl font-black text-slate-800 tracking-tight">{t.reviewTitle}</h2><p className="text-slate-500 mt-2 font-black text-lg">{t.reviewSubtitle}</p></div>
          <div className="bg-indigo-900 p-10 rounded-[48px] shadow-2xl space-y-6 text-white">
            <div className="flex items-center gap-4"><div className="bg-white/10 p-4 rounded-3xl"><Bookmark className="w-8 h-8 text-white" /></div><div className="flex-1"><label className="block text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-2">{t.templateName}</label><input type="text" value={newTemplateName} onChange={(e) => setNewTemplateName(e.target.value)} placeholder="e.g. EMEA Monthly VAT Pipeline" className="w-full bg-indigo-950/50 border border-white/10 px-8 py-5 rounded-[28px] text-xl font-black focus:ring-8 focus:ring-indigo-500/30 outline-none transition-all" /></div></div>
            <div className="flex gap-4 pt-4"><button onClick={() => setStep(5)} className="flex-1 px-10 py-5 border-2 border-white/10 text-white font-black rounded-[32px] hover:bg-white/10 transition-all uppercase tracking-widest text-xs">&larr; Back</button><button onClick={handleSaveTemplate} disabled={!newTemplateName} className="flex-[2] bg-white text-indigo-900 px-10 py-5 rounded-[32px] font-black shadow-2xl hover:bg-indigo-50 transition-all transform hover:-translate-y-2 disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-3 uppercase tracking-widest"><Save className="w-6 h-6" />{t.saveFinish}</button></div>
          </div>
        </div>
      )}
